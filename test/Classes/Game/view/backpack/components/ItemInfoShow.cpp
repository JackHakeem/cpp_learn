...
bool ItemInfoShow::initWithBagGoods(BagGoods* pBagGoods, BagView* bagView)
{
	//
	if (!pBagGoods || !bagView || !pBagGoods->myInfo)
		return false;
	//
	dieCount = 0.0f;
	//
	...
	//
	schedule(schedule_selector(ItemInfoShow::tick));
	//
	GoodsInfo* goodInfo = pBagGoods->myInfo;
	...
	*/
	...
	CCPoint bagGoodsPos = pBagGoods->getPosition();
	CCPoint __containerPosition = bagView->getPosition();
	CCPoint p = ccp(bagGoodsPos.x+__containerPosition.x+10, bagGoodsPos.y+__containerPosition.y);

	CCSize bagGoodsSize = pBagGoods->getContentSize();
	...
	{//Items on the right
			this->setPosition(ccp(POSX(p.x-bagGoodsSize.width/2-m_alert->getContentSize().width+4),  
				POSX(p.y-m_alert->getContentSize().height+40)));
		}
		else
		{//显示在右
			this->setPosition(ccp(POSX(p.x+bagGoodsSize.width/2),  
				POSX(p.y-m_alert->getContentSize().height+40)));
		}
	}
	else
...
void ItemInfoShow::tick(ccTime dt)
{
...
void ItemInfoShow::clickhandle(CCObject* pSender)
{
	return;
}

...
bool AccountSettingLayer::init()
{
	...
	//accountbtn
	{
		...
		if(pNormalSprite && pPressedSprite)
		{
			...
			CCLabelTTF* pLabel = CCLabelTTF::labelWithString(ValuesUtil::Instance()->getString("SET003").c_str(), CCSizeMake(POSX(230), POSX(FontSize)),CCTextAlignmentLeft, "Arial", POSX(FontSize));
			if (pLabel)
			{

...
bool CapacityPanel::init()
{
	do 
	{
		...
		CCLabelTTF* pFightLabel = CCLabelTTF::labelWithString(pVU->getString("RK005").c_str(), CCSizeMake(POSX(200), POSX(26)), CCTextAlignmentCenter, g_sSimHeiFont, POSX(26));
...
void CapacityPanel::updateRankList(const std::vector<RankItemVo>& array)
{
	do 
	{
		...
		for (ix = array.begin(); ix != array.end(); ++ix)
		{
			...
			CCLabelTTF* pRankLabel = CCLabelTTF::labelWithString(msg, CCSizeMake(POSX(100), 
				POSX(40)), CCTextAlignmentCenter, g_sSimHeiFont, POSX(25));
			if (pRankLabel)
			{
...
void CapacityPanel::updateMyRank(int rank)
{
	do 
	{
		ValuesUtil *pVU = ValuesUtil::Instance();
...
void CapacityPanel::updateEquipments(const std::vector<EquipItemVo>& array)
{
	_pFigureView->updateEquipments(array);
}

void CapacityPanel::updateFigureData(const RankItemVo& data)
{
	_pFigureView->updateFigureData(data);
}

void CapacityPanel::startListener()
{
	if (_pListView)
		_pListView->addEventListener(RankingListView::RANK_ITEM_SELECTED, this, callfuncND_selector(CapacityPanel::onListItemSelected));
}

void CapacityPanel::stopListener()
{
	if (_pListView)
		_pListView->removeEventListener(RankingListView::RANK_ITEM_SELECTED, this, callfuncND_selector(CapacityPanel::onListItemSelected));
}

void CapacityPanel::onListItemSelected(cocos2d::CCNode* node, void* data)
{
	do 
	{

...
CC_NS_BEGIN
	...
	CLogger::CLogger() : m_strPath(),m_strFile()
    {
    }
    ...
    CLogger& CLogger::NewLine(int nFileType/* = 0*/)
    {
        ...
        if(bOpen)
        {
			...
			time_t tlast = tNow - 300;
            tm * pTm_last = localtime(&tlast);
			memset(szTemp, 0, sizeof(szTemp));
			strftime(szTemp, sizeof(szTemp), pszFormat.c_str(), pTm_last);
